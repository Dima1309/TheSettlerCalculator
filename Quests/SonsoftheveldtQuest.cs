...
{
	internal class SonsoftheveldtQuest : Quest
	{
		#region Constants
		...
		private const int STONE_THROWER = 6;
		private const int RANGER = 7;
		private const int SKUNK = 8;
		private const int ONE_EYED_BERT = 9;
		#endregion

		#region Constructor
		internal SonsoftheveldtQuest()
		{
			Name = Resources.QUEST_SONS_OF_THE_VELDT;
			Units = InitializeUnits().ToArray();
			Camps = InitializeCamps().ToArray();

			IconPath = "TheSettlersCalculator.Quests.Icons.Sohne_der_Steppe.png";
		}
		#endregion
		...
		private static List<Unit> InitializeUnits()
		{
			...
			units.Add(EnemyUnits.Units[(int)EnemyUnitsEnum.CHUCK]);
		...
		private static List<Camp> InitializeCamps()
		{
			...
			camp.Counts.Add(ROUGHNECK, 50);
			camp.Counts.Add(SKUNK, 1);
			camp.Counts.Add(ONE_EYED_BERT, 2);
			camp.Counts.Add(GUARD_DOG, 50);
			camp.CampType = CampType.Boss;
			result.Add(camp);
